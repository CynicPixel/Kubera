...
SharedCacheManager::CacheSnapshot MarketImpactModel::get_cache_snapshot() const {
    return shared_cache_.get_snapshot();
}

void MarketImpactModel::update_shared_cache(const orderbook::OrderBook& order_book) const {
    shared_cache_.update_from_orderbook(order_book);
}

void MarketImpactModel::update_shared_cache_from_snapshot(const OrderBookSnapshot& snapshot) const {
...
// ✅ SHARED: Main impact calculation with shared cache
MarketImpactModel::ImpactComponents MarketImpactModel::calculateImpact(
    ...
    try {
        ...
        if (cache_snapshot.is_valid(ob_version, current_time_ns)) {
            ...
            return calculate_impact_from_shared_cache(quantity, cache_snapshot.data, is_buy);
        }
        
        // Cache miss - update shared cache with atomic snapshot and calculate
        ...
        if (fresh_cache.data) {
            return calculate_impact_from_shared_cache(quantity, fresh_cache.data, is_buy);
        }
        
        // Fallback to snapshot calculation
        return calculate_impact_from_snapshot(quantity, snapshot, is_buy);
...
// ✅ SHARED: Fast calculation with shared cache
MarketImpactModel::ImpactComponents MarketImpactModel::calculate_impact_fast(
    double quantity,
    const orderbook::OrderBook& order_book,
    bool is_buy
) const {
    try {
        ...
        logger_.error("Failed to calculate market impact (fast): {}", e.what());
        return ImpactComponents(0.0, 0.0, 0.0);
    }
}

// ✅ NEW: Calculate impact from consistent snapshot
MarketImpactModel::ImpactComponents MarketImpactModel::calculate_impact_from_snapshot(
    double quantity,
    const OrderBookSnapshot& snapshot,
    bool is_buy
) const {
    double bid_depth = calculate_effective_depth_from_snapshot(snapshot, false);
    double ask_depth = calculate_effective_depth_from_snapshot(snapshot, true);
...
// ✅ SHARED: Impact calculation from shared cache
MarketImpactModel::ImpactComponents MarketImpactModel::calculate_impact_from_shared_cache(
    double quantity,
    ...
    // Fast path for small quantities using pre-computed curves
    if (quantity < 10.24 && curves_valid_.load(std::memory_order_acquire)) {
        size_t index = static_cast<size_t>(quantity * 100.0);
        index = std::min(index, CACHE_SIZE - 1);
        
        double effective_depth = is_buy ? cache->askDepth : cache->bidDepth;
        effective_depth = std::max(effective_depth, 1.0); // Prevent division by zero
        
        // Adjust depth based on imbalance
        double depth_adjustment = 1.0 + (is_buy ? -cache->imbalance : cache->imbalance) * 0.5;
        effective_depth *= std::max(0.1, depth_adjustment);
        
        double immediate = cache->spread * 0.5;
        double permanent = permanent_impact_curve_[index] * cache->midPrice / std::sqrt(effective_depth);
        double temporary = temporary_impact_curve_[index] * cache->midPrice / std::pow(effective_depth, 0.375);
    ...
    // Full calculation for large quantities
    double effective_depth = is_buy ? cache->askDepth : cache->bidDepth;
    return calculate_impact_direct(
        quantity, cache->midPrice, cache->spread, effective_depth,
        cache->volatility, cache->imbalance, cache->bidDepth, cache->askDepth, is_buy
    );
}
...
MarketImpactModel::ImpactComponents MarketImpactModel::calculate_impact_hft(
    double quantity,
    double mid_price,
    double spread,
    double volatility,
    ...
    return calculate_impact_from_atomics(
        quantity, mid_price, spread, volatility, imbalance,
        ask_prices, ask_quantities, bid_prices, bid_quantities, is_buy
    );
}
...
// ✅ SHARED: Effective depth calculation from shared cache
double MarketImpactModel::calculate_effective_depth_from_shared_cache(const SharedMarketCache* cache, bool is_buy) const {
    return is_buy ? cache->askDepth : cache->bidDepth;
}

// ✅ NEW: Effective depth calculation from atomic snapshot
...
void MarketImpactModel::update_parameters(const ModelParameters& new_params) {
    update_parameters_internal(new_params);
}

void MarketImpactModel::update_parameters_internal(const ModelParameters& new_params) {
    {
...
void MarketImpactModel::setLiquidityFactor(double factor) {
    ModelParameters new_params = get_parameters();
    new_params.liquidity_factor = factor;
    update_parameters_internal(new_params);
}

void MarketImpactModel::setPriceImpactDecay(double decay) {
    ModelParameters new_params = get_parameters();
    new_params.price_impact_decay = decay;
    update_parameters_internal(new_params);
}
...
double MarketImpactModel::getLiquidityFactor() const {
    std::shared_lock<std::shared_mutex> lock(parameters_mutex_);
    return parameters_.liquidity_factor;
}

double MarketImpactModel::getPriceImpactDecay() const {
    std::shared_lock<std::shared_mutex> lock(parameters_mutex_);
    return parameters_.price_impact_decay;
}

// ✅ SHARED: Learning methods use atomic snapshots and shared cache
...
void MarketImpactModel::process_impact_records_batch() const {
    ...
    if (!batch.empty()) {
        // ✅ FIXED: Process entire batch under single lock
        {
        ...
        // ✅ FIXED: Calibrate outside of lock to prevent deadlock
